...
if(isset($_POST['create'])){
    ...
    $get_user = mysqli_query($koneksi, "SELECT * FROM tb_admin where nama_admin='$nama_admin'");
    if(mysqli_num_rows($get_user)>0){
        echo "<script>alert('Username sudah digunakan!');
		window.location.replace('create.php')</script>";
    }else{
        $sql = mysqli_query($koneksi,"INSERT INTO tb_admin VALUES('','$nama_admin','$username','$password',NOW())");
        if ($sql) {
